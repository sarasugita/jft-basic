...
import {
  defaultState, state, appBootstrapState, loadState, saveState,
  shouldBlockOnQuestions, exitToHome, goIntro,
} from "./state/appState";
import {
  ...
  getActiveTestType, getSessionTestType, getActivePassRate, getPassRateForVersion,
...
import {
  ...
  buildLatestAttemptMapByStudent, getScoreRateFromAttempt,
...
import {
  authState, studentPanelEntryUserId, refreshAuthState, registerAuthStateListener,
} from "./state/authState";
import { resetSessionScopedState } from "./state/index";
...
function renderTestSelect(app) {
  ...
  const attendanceHistoryHtml = showAttendanceHistory
    ? (() => {
        if (!authState.session) {
  ...
  if (showAttendanceHistory) {
    ...
    const closeBtn = app.querySelector("#applicationDetailClose");
