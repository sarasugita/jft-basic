...
// - Updates the current student's password using the service role client.
// - Clears the force_password_change flag on the public profile after success.
...
serve(async (req) => {
  ...
  const { data: profile, error: profileError } = await adminClient
    ...
    .single();
  if (profileError || !profile) return unauthorized("Profile not found");
  if (profile.role !== "student") return unauthorized("Student account only");
  ...
  if (authUpdateError || !authUpdateData?.user) {
    return bad(authUpdateError?.message ?? "Failed to update password");
  }
  ...
  if (profileUpdateError) {
    return bad("Profile update failed", { detail: profileUpdateError.message });
  }

  return ok({
    ok: true,
    user_id: callerUserData.user.id,
  });
});
